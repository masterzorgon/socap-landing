...
import { ChevronDownIcon, ChevronLeftIcon, ListBulletIcon, ShareIcon } from '@heroicons/react/16/solid'
...
export default async function BlogPost({
	...
	// Extract h2 sections from the content
	const extractSections = (body: any) => {
		const sections: Array<{ id: string; title: string }> = []

		if (!body || !Array.isArray(body)) return sections

		body.forEach((block: any) => {
			if (block.style === 'h2' && block.children && Array.isArray(block.children)) {
		...
		<main className="overflow-hidden">
			...
			<Container>
						...
						<div className="flex flex-wrap gap-2">
							{post.categories.map((category) => (
								<Link
									...
									className="rounded-lg border border-dotted border-gray-300 bg-gray-50 hover:bg-gray-100 text-sm/6 px-2 font-medium text-gray-500"
								>
									{category.title}
								</Link>
							))}
				...
				<div className="mt-16 grid grid-cols-1 gap-8 pb-24 lg:grid-cols-[15rem_1fr] xl:grid-cols-[15rem_1fr_15rem]">
					{/* sits on the left of the screen (large screen) */}
					<div>
						<div className="flex flex-wrap items-center gap-8 max-lg:justify-between lg:flex-col lg:items-start">
							...
							<ShareButton title={post.title as string} />
						</div>

						<hr className="my-10 border-t border-gray-200" />

						<div className="relative">
							<TableOfContentsDropdown sections={sections} />
						</div>
					</div>

					{/* sits in the center of the screen (large screen) */}
